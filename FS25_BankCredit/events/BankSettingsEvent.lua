...
function BankSettingsEvent:run(connection)
    ...
    local prevLeverageRatio = s.leverageRatio
    ...
    if not connection:getIsServer() then
        BankSettings:saveToXMLFile()
    end

    if connection:getIsServer() then
        for _, id in ipairs(BankSettings.menuItems) do
            local ctrl = BankSettings.CONTROLS[id]
            if ctrl ~= nil then
                ctrl:setState(BankSettings.getStateIndex(id, s[id]))
            end
        end
    end
    ...
    if s.leverageRatio ~= prevLeverageRatio then
