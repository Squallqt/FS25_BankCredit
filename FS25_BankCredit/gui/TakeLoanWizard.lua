...
function TakeLoanWizard:reset(farmId)
    ...
    if self.inputAmount   then self.inputAmount:setText(tostring(BankCredit.toDisplayMoney(self.selectedAmount))) end
...
function TakeLoanWizard:onAmountEnterPressed(element)
    local value   = BankCredit.fromDisplayMoney(tonumber(element:getText() or "") or 0)
    local clamped = math.max(TakeLoanWizard.AMOUNT_MIN, math.min(value, self.amountLimit))
    self.selectedAmount = clamped
    element:setText(tostring(BankCredit.toDisplayMoney(clamped)))
    self:updateStep1()
end
